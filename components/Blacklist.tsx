  ...
  return (
    <div className="w-full py-16 px-6 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col items-center text-center mb-12">
        <div className="w-20 h-20 bg-black border-2 border-red-600 rounded-full flex items-center justify-center mb-6 shadow-[0_0_30px_rgba(220,38,38,0.5)]">
      ...
      <div className="flex justify-center mb-12">
        <button 
          ...
          className="bg-red-950 hover:bg-red-900 border border-red-600 text-red-100 px-8 py-4 rounded-xl font-bold text-lg flex items-center gap-3 transition-all hover:scale-105 shadow-lg shadow-red-900/30"
        >
          ...
          <table className="w-full text-left">
            <thead className="bg-red-950/50 text-red-200 uppercase text-xs font-bold tracking-wider">
              <tr>
                ...
                <th className="px-6 py-4">{t.table.discord}</th>
              ...
              {MOCK_BLACKLIST.length > 0 ? (
                MOCK_BLACKLIST.map((entry, index) => (
                  <tr key={index} className="hover:bg-red-900/10 transition-colors">
                    ...
                    <td className="px-6 py-4 text-slate-400 font-mono text-sm">{entry.discord}</td>
                    <td className="px-6 py-4 text-red-300">{entry.reason}</td>
                    <td className="px-6 py-4 text-slate-500 text-sm">{entry.date}</td>
                    <td className="px-6 py-4 text-right">
                      <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-950 text-red-500 border border-red-800">
                        {entry.status}
                      </span>
                    </td>
                  </tr>
